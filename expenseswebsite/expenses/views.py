...
def add_expense(request):
    ...
    if request.method=='POST':
         ...
         income_sum= UserIncome.objects.filter(owner=request.user)
         insum= income_sum.aggregate(Sum('amount'))
         expenses = Expense.objects.filter(owner=request.user)
         sum = expenses.aggregate(Sum('amount'))
         messages.warning(request,'total income is ' + str(insum['amount__sum']))
         messages.warning(request,'total expenses is ' + str(sum['amount__sum']))
